...
src = r"D:\PROJECTS\FROGFACE-SPACE\refs\about-panels"
out = os.path.join(src, "_sheet-about.png")
files = sorted(f for f in os.listdir(src) if f.lower().endswith(".png") and not f.startswith("_"))
THUMB_W, PAD, LABEL_H, COLS = 460, 14, 26, 2
try:
...
d.text((PAD, 12), "ABOUT scroll-comic (%d panels)" % len(thumbs), fill=(182, 255, 58), font=font)
...
print("saved:", out, "| panels:", len(thumbs))
